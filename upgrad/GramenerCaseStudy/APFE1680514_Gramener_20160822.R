...
#----Outliers Treatment--------------------------------------------------------
#-Based on teh above graphs we clearly see a need to perform soem outlier cleaning
#-especially for the Annual income data
loan_ann_inc_outliers <- boxplot.stats(loan_final_data$annual_inc)$out
summary(loan_ann_inc_outliers)

loan_fund_amnt_outliers <- boxplot.stats(loan_final_data$funded_amnt)$out
summary(loan_fund_amnt_outliers)

loan_no_outlier_data <- loan_final_data[!loan_final_data$annual_inc %in% loan_ann_inc_outliers,]
loan_no_outlier_data <- loan_no_outlier_data[!loan_no_outlier_data$annual_inc %in% loan_fund_amnt_outliers,]

summary(loan_no_outlier_data)    
...
grid.arrange(int_rate_plot,
             ...
             ownership_plot ,
             loan_status_1_plot,
             ncol = 2, top = "Catregorical Data Plots")
#
...
#loan_amnt       1.000       0.979      0.469     0.088
#funded_amnt     0.979       1.000      0.464     0.085
#annual_inc      0.469       0.464      1.000    -0.014
#dti             0.088       0.085     -0.014     1.000
