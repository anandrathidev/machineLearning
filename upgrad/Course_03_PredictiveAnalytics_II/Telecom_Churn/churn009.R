...
factor_plots_1 <- ggplot(churn, aes(x=Churn,  group = gender)) + 
  ...
  stat_count(aes(y = ..count..*0.5, label = paste( round(prop.table(..count..) * 100)  ), "%"), vjust = -1, geom = "text", position = "identity", color ="black")

factor_plots_2 <- ggplot(churn, aes(x=Churn,  group = SeniorCitizen)) + 
  geom_bar(aes(y = ..count.., fill = factor(SeniorCitizen)), stat="count") + 
  stat_count(aes(label = paste( round(prop.table(..count..) * 100)  ), "%"), vjust = 1, geom = "text", position = "identity", color ="black")
...
factor_plots_14 <- ggplot(churn, aes(x=Churn,  fill= TechSupport, group = TechSupport)) + 
  geom_bar(stat="count")  + 
  stat_count(aes(label = paste( round((..count..)/sum(..count..) * 100)  ), "%"), vjust = (..count..), geom = "text", color ="black")
...
grid.arrange(factor_plots_13,
 ...
 # 
# 
# #--We have 17 Categorical and 3 numerical variables [excluding Customer ID]
# str(churn)
# ggplot(churn, aes(gender, fill=churn$Churn)) + geom_bar()
# ggplot(churn, aes(SeniorCitizen, fill=churn$Churn)) + geom_bar()
# ggplot(churn, aes(Partner, fill=churn$Churn)) + geom_bar()
