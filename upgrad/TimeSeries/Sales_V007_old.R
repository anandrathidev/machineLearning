...
par(mfrow = c(1,1))
plot.forecast(EU.Consumer.Sales.forcast_12, main = "EU.Consumer.Sales.forcast")

summary(EU.Consumer.Sales.forcast_12)
...
acf(EU.Consumer.Sales.forcast_12$residuals, lag.max=20)
Box.test(EU.Consumer.Sales.forcast_12$residuals, lag=20, type="Ljung-Box")


par("mar")
par(mar=c(3,5.1,2,2.1))
par(mfrow = c(1,3))
plot.ts(EU.Consumer.Sales.forcast_12$residuals)            # make time plot of forecast errors
plotForecastErrors(EU.Consumer.Sales.forcast_12$residuals) # make a histogram
...
resDF <- as.data.frame(cbind(YrMon=rownames(EU.Consumer.Sales.forcast_12_df),
                             EU.Consumer.Sales = EU.Consumer.Sales.forcast_12_df$`Point Forecast`, 
                             EU.Consumer.Quantity = EU.Consumer.Quantity.forcast_12_df$`Point Forecast`,
                             EU.Corporate.Sales = EU.Corporate.Sales.forcast_12_df$`Point Forecast`,
                             EU.Corporate.Quantity = EU.Corporate.Quantity.forcast_12_df$`Point Forecast`,
                             APAC.Consumer.Sales = APAC.Consumer.Sales.forcast_12_df$`Point Forecast`, 
                             APAC.Consumer.Quantity = APAC.Consumer.Quantity.forcast_12_df$`Point Forecast`,
                             APAC.Corporate.Sales = APAC.Corporate.Sales.forcast_12_df$`Point Forecast`, 
                             APAC.Corporate.Quantity = APAC.Corporate.Quantity.forcast_12_df$`Point Forecast`,
                             LATAM.Consumer.Sales = LATAM.Consumer.Sales.forcast_12_df$`Point Forecast`,
                             LATAM.Consumer.Quantity = LATAM.Consumer.Quantity.forcast_12_df$`Point Forecast`))
resDF[which(as.yearmon(resDF$YrMon) > as.yearmon('Nov 2014') ), ]
...
ggplot(resDF, aes(x = YrMon)) + 
  ...
  xlab("Sales/Months")  +  scale_colour_manual(values=c("red","green","blue", "black", "purple")) 
