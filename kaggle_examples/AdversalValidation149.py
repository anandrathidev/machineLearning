...
num_of_features = int(data.shape[1] * 0.20)+1
...
def DecomposedFeatures(train,  test,
    ...
    if use_pls>0.0:
        ...
        pls_results = srp.fit(total)
        pls_results_train = srp.transform(train)
        pls_results_test = srp.transform(test)
...
def get_training_set_with_test_set_similarity_predictions(X_train, Y_train, X_test):
    ...
    xclf=CatBoostClassifier(iterations=171,
                           depth=6,
                           border_count=125,
                           learning_rate=0.055, loss_function='Logloss',
                           thread_count=7)
    ...
    rfcls = RandomForestClassifier(n_estimators = 151, n_jobs = -2)
...
train_set_with_predictions_for_test_set_similarity, X_test =  get_training_set_with_test_set_similarity_predictions(X_train=trainDecomp,
                                                                                                                    Y_train=YBC,
                                                                                                                    X_test=testDecomp)
...
def AdversarialTrainVal(train, test ):
    ...
    Xtrain = train[ train["prob_test"] > 0.65 ]
    Xtrain = train[ train["prob_test"] > 0.60 ]
    Xtrain = Xtrain.append(train[ train["prob_test"] > 0.55 ])
    Xtrain = Xtrain.append(train[ train["prob_test"] > 0.51 ])
    Xtrain = Xtrain.append(train[ train["prob_test"] > 0.50 ])
    Xtrain = Xtrain.append(train[ train["prob_test"] > 0.50 ])
    Xtrain = Xtrain.append(train[ train["prob_test"] > 0.49 ])
    Xtrain = Xtrain.append(train[ train["prob_test"] > 0.45 ])
    Xtrain = Xtrain.append(train[ train["prob_test"] > 0.40 ])
    #Xtrain = Xtrain.append(train)
    
    Xtrain = Xtrain.drop(["is_test", "prob_test"], 1)
    val = train[ train["prob_test"] > 0.49 ]
    val = val.append(train[ train["prob_test"] > 0.50 ])
    val = val.append(train[ train["prob_test"] > 0.55 ])
    val = val.append(train[ train["prob_test"] > 0.60 ])
    ...
    val = val.append(train[ train["prob_test"] > 0.85 ])
    val = val.append(train[ train["prob_test"] > 0.90 ])
    val = val.append(train[ train["prob_test"] > 0.91 ])
...
ypredavg = pd.DataFrame(dict(
    ...
    LGB1=ytest_lgbm1,
    AVG= (2*ytest_lgbm1 + ytest_catgbmx1 )/3  ) )
