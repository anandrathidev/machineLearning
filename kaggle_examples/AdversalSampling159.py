...
def Covariateshift(X_train, X_test):
    ...
    total = pd.DataFrame( total.fillna(0))

    x = total.drop(["is_test"], axis = 1)
    y = total.is_test
    print("Start cross-validating")
    rfcls = RandomForestClassifier(n_estimators = 5, n_jobs = -2)
    #x_train, x_test, y_train, y_test = train_test_split(x, y, stratify = y, test_size = 0.25, random_state = 42)    
    print("fit RF - " )
    drop_list=[]
    for i in X_train.columns:
        score = cross_val_score(rfcls, x,y,cv=2,scoring='roc_auc')
        if (np.mean(score) > 0.7):
            drop_list.append(i)
            print( " col {} score {}".format(i, score ))
    X_train= X_train.drop("is_test", 1)
    X_test = X_test.drop("is_test", 1)

    #X_train= X_train.drop(drop_list, axis=1)
    #X_test= X_test.drop(drop_list, axis=1)
    return drop_list

print("dataScaledLog {}".format(dataScaledLog.head(1)) )
print("testdataScaledLog {}".format(testdataScaledLog.head(1)) )
drop_list = Covariateshift(X_train=dataScaledLog, X_test=testdataScaledLog) #
...
def DecomposedFeatures(train,  test,
    ...
    np.concatenate([srp_results_train, grp_results_train, ica_results_train, pca_results_train, tsvd_results_train], axis=1)
    # Remove any NA
