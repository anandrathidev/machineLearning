...
#print(dataScaledLog.sort_values(by=1,axis=1).head())
#print(dataScaledLog.max(axis=1).head())
#print(dataScaledLog.min(axis=1).head())
#print(dataScaledLog.mean(axis=1).head())
#print(dataScaledLog.sum(axis=1).head())
#order = np.argsort(-dataScaledLog.values, axis=1)[:, :3]
#print(order[:3])
dfscdataScaledLog =  dataScaledLog.apply(lambda x: pd.Series(x.sort_values(ascending=False).values), axis=1)
testdataScaledLog =  testdataScaledLog.apply(lambda x: pd.Series(x.sort_values(ascending=False).values), axis=1)
fulldataScaledLog =  fulldataScaledLog.apply(lambda x: pd.Series(x.sort_values(ascending=False).values), axis=1)
...
regr.fit(dfscol, Y)
y_pred = regr.predict(dfscol)
trainrms = sqrt(mean_squared_error( Y, y_pred))
print("lin  trainrms {}".format(  trainrms ) )
...
def CreateNF(x):
    ...
    newFdata=newFdata.assign(LIN=y_pred)
    topic_dist_unnormalized = np.matrix(lda.transform(x))
    newFdata=newFdata.assign(LDA=topic_dist_unnormalized)
...
fulldatanewF = CreateNF(fulldataScaledLog)fulldatanewF= pd.DataFrame(rscaler.transform( fulldatanewF ))
...
# In[37]:
datanewF = dfscdataScaledLog 
testdatanewF =  testdataScaledLog
fulldatanewF =  fulldataScaledLog
...
def TrainSimilarToTest(X_train, Y_train, X_test):
    ...
    xclf=CatBoostClassifier(iterations=11,
                           depth=3,
                           border_count=11,
    ...
    rfcls = RandomForestClassifier(n_estimators = 11, n_jobs = -2)
    ...
    X_train = X_train.assign( prob_test = (xdf_Prob[[1]]))
    ...
    X_test = X_test.assign( prob_test = (xdf_Prob[[1]]))
...
def AdversarialTrainVal(train, test ):
    ...
    Xtrain = pd.DataFrame( train.nlargest(int(train.shape[0]*0.51), 'prob_test'), columns=train.columns)
    ...
    X_data, X_test, Y_data, y_test = train_test_split(X_xres, X_xres.target,
                                                    ...
                                                    test_size=0.65, random_state=44)
    ...
    val = train.nsmallest(int(train.shape[0]*0.85), 'prob_test')
...
valDecomp = np.concatenate([valDecomp, x_val], axis=1)
testDecomp = np.concatenate([testDecomp, x_test], axis=1)
